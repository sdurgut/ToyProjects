...
# print len(trUserRatings), len(trUserRatings[1]), trUserRatings[100]
...
# print "Simple User-based prediction RMSE:", rmse(actualRatings, meanUserRatings)
# print "Simple Movie-based prediction RMSE:", rmse(actualRatings, meanMovieRatings)
# print "Simple prediction RMSE:", rmse(actualRatings, meanRatings)
...
# print [x[0] for x in trainingSet]
# print trUserRatings[1]



# Use userRatings from training set, namely use trUserRatings set
def similarity(u, v, userRatings):
	uMeanUserRating = trmeanUserRatings[u-1]
	vMeanUserRating = trmeanUserRatings[v-1]
	nominatorSum = 0
	uDenominatorSum = 0
	vDenominatorSum = 0

	uMovieList = [x[0] for x in trUserRatings[u-1].items()]
	vMovieList = [x[0] for x in trUserRatings[v-1].items()]
	for umovie in uMovieList:
		if umovie in vMovieList:
			r_u = trUserRatings[u-1][umovie]
			r_v = trUserRatings[v-1][umovie]
...
print similarity(102,200,trUserRatings)
